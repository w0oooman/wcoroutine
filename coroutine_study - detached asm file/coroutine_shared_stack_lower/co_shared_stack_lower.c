...
static void
save_stack(struct coroutine *C, char *top) {
	char dummy;
	assert(top - &dummy <= STACK_SIZE);
	if (C->s_cap < top - &dummy) {
		free(C->s_stack);
		C->s_cap = top - &dummy;
		C->s_stack = malloc(C->s_cap);
	}
	C->s_size = top - &dummy;
	memcpy(C->s_stack, &dummy, C->s_size);
}
...
coroutine_resume(struct schedule * S, int id) {
	...
	case COROUTINE_SUSPEND:
		memcpy(S->stack + STACK_SIZE - C->s_size, C->s_stack, C->s_size); /* S->stack + STACK_SIZE - C->s_size == &dummy */
		S->curid = id;
		C->status = COROUTINE_RUNNING;
		co_swapctx(&S->main, &C->ctx);
		break;
...
coroutine_yield(struct schedule * S) {
	...
	struct coroutine * C = S->co[id];
	save_stack(C, S->stack + STACK_SIZE);
